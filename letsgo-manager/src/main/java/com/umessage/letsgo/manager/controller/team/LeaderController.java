...
//        TravelAgencyEntity travel = travelService.getByUserId(user.getId());
    ...
    public String leaderPreview(Long id,Model model){
        LeaderEntity leaderEntity = leaderService.getLeader(id);
        Page<ScheduleEntity> traveledList = scheduleService.getSchedulesByUserAndStatus(Constant.TRAVELED,id);//已出行
        Page<ScheduleEntity> prepareTravelList = scheduleService.getSchedulesByUserAndStatus(Constant.PREPARE_TRAVEL,id);//即将出行
        Page<ScheduleEntity> travelingList = scheduleService.getSchedulesByUserAndStatus(Constant.TRAVELING,id);//正在出行
